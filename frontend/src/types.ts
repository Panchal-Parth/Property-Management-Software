...
export type Unit = { id: number; property_id: number; label: string; floor: string; unavailable: number; archived: number }
...
export type Monthly = { id: number; property_id: number; unit_id: number | null; month: string; amounts: number[]; notes: string }
...
export type State = { today: string; settings: { name: string; email: string; currency: string; timezone: string }; categories: { id: number; name: string; kind: string }[]; properties: Property[]; units: Unit[]; tenants: Tenant[]; leases: Lease[]; records: Monthly[]; documents: Document[]; deposits: Deposit[] }
